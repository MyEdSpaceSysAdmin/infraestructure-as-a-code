resource "google_service_account" "cloud_composer_environment_service_account" {
  ...
  display_name = "Cloud Composer environment service account for PROD"
}
...
resource "google_project_iam_member" "cloud_composer_environment_service_account" {
  ...
  depends_on = [ google_service_account.cloud_composer_environment_service_account ]
}
...
# TODO: Manage Composer via Terraform when Composer 3 images are supported 
